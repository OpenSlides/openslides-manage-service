...
// These tests simlate the user calling without flags. But we cannot give Setup a null pointer
// for tpl, so we give the default value, which is normally ensured by cobra.
var defaultDeploymentTemplate = "docker-compose"
...
func TestSetupCommon(t *testing.T) {
	...
	t.Run("running setup.Setup() and create all stuff in tmp directory", func(t *testing.T) {
		if err := setup.Setup(testDir, false, &defaultDeploymentTemplate, nil); err != nil {
	...
	t.Run("running setup.Setup() with force flag with changing existant files", func(t *testing.T) {
		if err := setup.Setup(testDir, true, &defaultDeploymentTemplate, nil); err != nil {
			t.Fatalf("running Setup() failed with error: %v", err)
		}
		secDir := path.Join(testDir, setup.SecretsDirName)
		testContentFile(t, testDir, "docker-compose.yml", defaultDockerComposeYml())
...
func TestSetupNonExistingSubdirectory(t *testing.T) {
	...
	t.Run("running setup.Setup() and give a previously not existing subdirectory", func(t *testing.T) {
		...
		if err := setup.Setup(dir, false, &defaultDeploymentTemplate, nil); err != nil {
			t.Fatalf("running Setup() failed with error: %v", err)
		}
...
services:
  proxy:
    tag: 2.0.0
`
		cfgPath := path.Join(testLibDir, "ext-conf")
		if err := os.WriteFile(cfgPath, []byte(customConfig), os.ModePerm); err != nil {
			t.Fatalf("writing custom config failed: %v", err)
		}
		myFileName := "my-filename-ooph1OhShi.yml"
		c := make([]string, 1)
		c[0] = cfgPath
		if err := setup.Setup(testDir, false, &defaultDeploymentTemplate, &c); err != nil {
			t.Fatalf("running Setup() failed with error: %v", err)
		}
		secDir := path.Join(testDir, setup.SecretsDirName)
...
func TestSetupNoDirectory(t *testing.T) {
	...
	err := setup.Setup("setup_test.go", false, &defaultDeploymentTemplate, nil)
